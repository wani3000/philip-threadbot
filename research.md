...
- the repository now writes `insights_media_ids` into `generation_notes` for threaded publishes, backfills them from successful `publish_attempts`, and aggregates accessible segment metrics into one post-level snapshot
- one legacy single-post publish still has no alternate insight-capable ID from Threads, so it remains untracked without breaking the dashboard
...
- Threads insights endpoints, richer home analytics, and library reuse tooling are implemented in code. Production account-level syncing is verified, and threaded posts now support post-level snapshots through stored `insights_media_ids` plus segment aggregation. The only residual limitation is a legacy single-post publish whose original Threads ID does not support the insights endpoint.
...
This project started from a blank repository and now has a working MVP dashboard/API foundation, pre-credential demo mode, launch-readiness scaffolding, live Supabase production wiring, real Google admin login, Anthropic-backed draft generation, Telegram preview delivery, real Threads publishing, structured Philip source-material seeding, a multi-post thread workflow, a monthly rescheduling calendar, reusable library actions, and dashboard-facing insight summaries. The MVP launch path is complete and the current operating policy is intentionally low-cost: one thread every two days, fixed theme rotation, recent-material avoidance, and local duplicate rejection without extra LLM validation calls. Threads account-level insights and threaded post-level insights are both wired in production; the only remaining caveat is that some legacy single-post publishes may not expose an alternate insights-capable identifier through the Threads API.
